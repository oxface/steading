...
- **Gate**: any non-graduated concept in what you're about to write → stop, hand it to the user, coach via the guided-rep loop in `docs/training/README.md` (Frame → Sample → Build → Review → free-text Quiz; rep 2+ = Review + Quiz only). Graduated concepts you may generate freely; user still reviews.
